...
class Event
  ...
  def self.create_list_from_csv(csv_filename)
    events = CSV.read(csv_filename, headers: true, encoding: 'UTF-8').map(&:to_h).map.with_index do |row, index|
      ...
      self.new(row['id']&.strip.to_i, type_str, row['title']&.strip, row['place']&.strip, period, row['details']&.strip, "images/#{image_dir}/#{row['path']}.#{row['ext']}", nil)
    end
    {
...
def combine_day_and_time_str(day_str, time_str)
  if !day_str.nil? && !time_str.nil?
    ...
    Time.local(2018, month, day, hour, minute)
  end
end
...
class Period
  def self.create_from_day_and_time(start_day_str, start_at_str, end_day_str, end_at_str)
    if !start_day_str.nil?
      start_day = Date.new(2019, start_day_str[0..1].to_i, start_day_str[2..3].to_i)
      end_day = Date.new(2019, end_day_str[0..1].to_i, end_day_str[2..3].to_i)
